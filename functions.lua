...
function Public.draw_minimap(icw, player, surface, position)
    ...
    if not frame.icw_auto_switch then
        frame.add({type = 'switch', name = 'icw_auto_switch', allow_none_state = false, left_label_caption = 'ON', right_label_caption = 'OFF'})
    end
    if not element then
        element =
        ...
        if not player.gui.left.icw_main_frame.icw_discard_button_name then
            local close_button = player.gui.left.icw_main_frame.add({type = 'button', name = 'icw_discard_button_name', caption = 'Close'})
